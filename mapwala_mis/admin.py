...
from .models import RequestForQuote, RFQSelection, RFQQuotation


# ──────────────────────────────────────────────
# INLINE: RFQ Selection
# ──────────────────────────────────────────────
...
# ──────────────────────────────────────────────
# INLINE: RFQ Quotation (Vendor Responses)
# ──────────────────────────────────────────────
class RFQQuotationInline(admin.TabularInline):
    model = RFQQuotation
    extra = 1
    fields = ("vendor", "status", "quotation_rate", "quotation_date")
    readonly_fields = ("created_at", "updated_at")
    show_change_link = True


# ──────────────────────────────────────────────
# MAIN ADMIN: Request For Quote
# ──────────────────────────────────────────────
...
class RequestForQuoteAdmin(admin.ModelAdmin):
    ...
    list_filter = ("status", "assembly_type", "quote_types", "created_at")
    search_fields = ("id", "order__order_id", "created_by__username")
    ordering = ("-created_at",)

    # FORM ORGANIZATION (VERY IMPORTANT FOR UX)
    fieldsets = (
        ("Basic Information", {"fields": ("order", "status", "created_by")}),
        ("RFQ Details", {"fields": ("quote_types", "assembly_type", "quantity")}),
        ("Delivery Details", {"fields": ("delivery_date", "delivery_address")}),
        ("Additional Info", {"fields": ("additional_requirements",)}),
        (
            "Timestamps",
            {
                "fields": ("created_at",),
    ...
    # INLINE CHILD TABLES
    inlines = [RFQSelectionInline, RFQQuotationInline]
...
# ──────────────────────────────────────────────
# RFQ SELECTION ADMIN (Optional standalone)
# ──────────────────────────────────────────────
@admin.register(RFQSelection)
class RFQSelectionAdmin(admin.ModelAdmin):
    list_display = ("id", "rfq", "item_type", "reference")
    list_filter = ("item_type",)
    search_fields = ("reference", "rfq__id")


# ──────────────────────────────────────────────
# RFQ QUOTATION ADMIN (Vendor Side)
# ──────────────────────────────────────────────
...
class RFQQuotationAdmin(admin.ModelAdmin):
    ...
    list_editable = ("status", "quotation_rate")

    fieldsets = (
        ("Basic Info", {"fields": ("rfq", "vendor", "status")}),
        ("Quotation Details", {"fields": ("quotation_rate", "quotation_date")}),
